//This achieves the multi-thread of bulr_x and blur_y
//derrivative_x and derrivative_y
...
//Added by Hao Wu for multi-thread 
struct thread_args_derrivative_x
{
    int cols;
    int row_s;
    int row_e;
...
void *derrivative_x(void *arguments)
{
    /****************************************************************************
    * derrivative in the x - direction.
    ****************************************************************************/
    struct thread_args_derrivative_x *args = arguments; 

    int cols = args->cols;
    int row_s = args->row_s;
    int row_e = args->row_e;
    short *smoothedim = args->smoothedim;
    short **delta_x = args->delta_x;
...
//Added by Hao Wu for multi-thread 
struct thread_args_derrivative_y
{
    int rows;
    int cols;
    int col_s;
...
void *derrivative_y(void *arguments)
{
    /****************************************************************************
    * derrivative in the x - direction.
    ****************************************************************************/
    struct thread_args_derrivative_y *args = arguments; 
        ...
        short int **delta_x, short int **delta_y)
{
    ...
    int iret[N_T];
    struct thread_args_derrivative_x args_derrivative_x[N_T];

    int row_per_t = rows/N_T;

    for(int i=0; i<N_T; i++)
    {
        args_derrivative_x[i].cols = cols;
        args_derrivative_x[i].row_s = row_per_t*i;
        args_derrivative_x[i].row_e = row_per_t*(i+1);
        args_derrivative_x[i].smoothedim = smoothedim;
        args_derrivative_x[i].delta_x = delta_x;
        iret[i] = pthread_create(&thread[i], NULL, &derrivative_x, &args_derrivative_x[i]);
    }
     ...
     * losing pixels.
     ****************************************************************************/
    struct thread_args_derrivative_y args_derrivative_y[N_T];

    int col_per_t = cols/N_T;

    for(int i=0; i<N_T; i++)
    {
        args_derrivative_y[i].rows = rows;
        args_derrivative_y[i].cols = cols;
        args_derrivative_y[i].col_s = col_per_t*i;
        args_derrivative_y[i].col_e = col_per_t*(i+1);
        args_derrivative_y[i].smoothedim = smoothedim;
        args_derrivative_y[i].delta_y = delta_y;
        iret[i] = pthread_create(&thread[i], NULL, &derrivative_y, &args_derrivative_y[i]);
    }
...
void *blur_x(void *arguments)
{
     ...
     ****************************************************************************/
    struct thread_args_blur_x *args = arguments; 
...
void *blur_y(void *arguments)
{
     ...
     ****************************************************************************/
    struct thread_args_blur_y *args = arguments; 
        ...
        short int **smoothedim)
{
    ...
    pthread_t thread[N_T];
    int iret[N_T];
    struct thread_args_blur_x args_x[N_T];
    ...
    struct thread_args_blur_y args_y[N_T];
    int row_per_t = rows/N_T;
